...
export function PlaylistProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (savedHistory) {
      const historyVideos = JSON.parse(savedHistory).map((video: any) => ({
        ...video,
        addedAt: new Date(video.addedAt),
  ...
  const addToHistory = (video: Video) => {
    ...
    setHistory((prev) => {
      ...
      const filtered = prev.filter((v) => v.id !== video.id)
      // Add to beginning and keep only last 30
      return [historyVideo, ...filtered].slice(0, 30)
    })
  }
  ...
  const handleVideoCompleted = () => {
    ...
    if (autoDeleteAfterPlayback) {
      // Remove current video from recent videos
      removeFromRecentVideos(currentVideo.id)

      // Remove current video from all playlists
      setPlaylists((prev) =>
        prev.map((playlist) => ({
          ...playlist,
          videos: playlist.videos.filter((v) => v.id !== currentVideo.id),
      ...
      // Get next video BEFORE removing current video
      const nextVideo = getNextVideo()

      if (nextVideo && nextVideo.id !== currentVideo.id) {
        setCurrentVideo(nextVideo)
      } else {
        // No more videos available, stop playback
        setCurrentVideo(null)
        setIsPlaying(false)
      }
    } else {
  ...
  const addToQueue = async (url: string): Promise<void> => {
    try {
      ...
      showNotification(`❌ ${error instanceof Error ? error.message : "Failed to add video"}`, "error")
      throw error
    }
  }
